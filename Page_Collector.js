...
// Profile: if one is provided, copy it into the run profile dir and launch from the copy
// so the source stays pristine. Otherwise start from a fresh empty profile dir.
...
function build_browser_args() {
    ...
    if (consent !== 'no-action') extension_paths.push(consent_extension_path);
    if (provided_extension_path) extension_paths.push(path.resolve(provided_extension_path));

    if (extension_paths.length > 0) {
        const joined = extension_paths.join(',');
        browser_args.push(`--load-extension=${joined}`, `--disable-extensions-except=${joined}`);
    }
...
function serialize_to(point_name, domain_output_directory, html_content) {
    ...
    if (!fs.existsSync(point_directory)) fs.mkdirSync(point_directory, { recursive: true });
...
async function crawl_domain(domain, browser) {
    ...
    if (!fs.existsSync(domain_output_directory)) fs.mkdirSync(domain_output_directory, { recursive: true });
    ...
    try {
        ...
        metadata.html_size_bytes.network_idle = serialize_to('network_idle', domain_output_directory, await page.content());

        metadata.success = true;

    } catch (error) {
        // load never fired (or another nav error): mark failed but still capture the exit-state DOM.
